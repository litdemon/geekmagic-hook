...
📷 **필요한 사진 ②**
> 책상 위에 SmallTV-Ultra가 시계를 표시하고 있는 실물 사진  
> (주변 개발 환경이 함께 보이면 좋음 — 키보드, 모니터 등)
...
📷 **필요한 사진 ⑤**
> SmallTV-Ultra 디스플레이에 7가지 GIF가 순서대로 재생되는 모습을 촬영한  
> 나란히 놓인 콜라주 사진 (또는 짧은 영상 클립의 캡처)  
> — 각 GIF별로 디스플레이 실물 사진 1장씩도 가능
...
- 픽셀 아트 느낌의 애니메이션이 책상 분위기에 잘 녹아듦

📷 **필요한 사진 ⑥ (메인 사진)**
> **시나리오 장면**: 모니터에는 Claude Code 터미널이 실행 중이고,  
> 옆에 놓인 SmallTV-Ultra에는 `permission.gif` 또는 `waiting.gif`가 표시된 모습  
> — 블로그 대표 이미지로 쓸 수 있는 구도
...
| # | 장면 | 필수 여부 |
|---|------|----------|
| ① | Claude Code 퍼미션 요청 멈춤 화면 (스크린샷) | 필수 |
| ② | 책상 위 SmallTV-Ultra 시계 모드 실물 | 필수 |
| ③ | curl API 호출 터미널 화면 (스크린샷) | 선택 |
| ④ | settings.json 훅 등록 코드 화면 (스크린샷) | 선택 |
| ⑤ | 7가지 GIF 재생 중인 디스플레이 콜라주 | 필수 |
| ⑥ | 모니터 + SmallTV-Ultra 함께 찍은 메인 사진 | 필수 |
| ⑦ | setup 명령 터미널 실행 화면 (스크린샷) | 선택 |

> 스크린샷은 포스팅 작성 시 직접 캡처 가능. **실물 사진 (②⑤⑥)이 핵심**이며  
> 먼저 준비되면 포스팅 초안 작성을 시작할 수 있습니다.
